...
class Affidavits extends StatefulWidget {
  ...
  Affidavits(this.applicant_id, this.image_name);

  @override
  _AffidavitsState createState() => _AffidavitsState();
}

class _AffidavitsState extends State<Affidavits> {
  String? _extension;
  String? profileType;
  ...
  List<String> affidavits_list = <String>[];
...
//      global.occupantImages;
//      print(global.occupantImages);
      house_hold = sharedPreferences.getStringList('affidavits');
    });
  }

  void _filePicker() async {
    await request.ifInternetAvailable();
    affidavits_list = <String>[];
    ...
    if(listFiles!=null){
      ...
      for (int j = 0; j < files.length; j++) {
        img.add(await MultipartFile.fromFile(files[j].path,
            filename: 'affidavits($j)'));
        img_paths.add(files[j].path);
        affidavits_list.add(files[j].path);
        print(_formData);
      }
      _formData['affidavits[]'] = img;
      _isLoadingSecondary = true;
      ...
      try {
        ...
        Map<String, dynamic> map = {
          ...
          'affidavits': jsonEncode(img_paths)
        };
        ...
        if (MyConstants.myConst.internet??false) {
          ...
          if (response.statusCode == 200) {
            setState(() {
              ...
              print(jsonResponse);
              print('yaha aya');
              var dependent_List = jsonResponse['data']['affidavits'];
              String? fileName;
              print('filename');
...
//            ){
                ...
                affidavits_id.add(newEntry);
              } else {
                affidavits_id.add(
                    'https://pngimage.net/wp-content/uploads/2018/06/files-icon-png-2.png');
              }
              sharedPreferences.setStringList("affidavits", affidavits_id);
    ...
    //   affidavits_list.add(listFiles[j].path);
    //   print(_formData);
    // }
...
//          "signature_date": selectedDate,
          'affidavits[]': await MultipartFile.fromFile(_image!.path,
              filename: 'household.jpg')
        });

        img_paths.add(_image!.path);
        affidavits_list.add(_image!.path);
        Map<String, dynamic> map = {
          "application_id": widget.applicant_id,
          'affidavits[]': jsonEncode(img_paths)
        };
